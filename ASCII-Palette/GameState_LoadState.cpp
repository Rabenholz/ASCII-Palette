...
GameState_LoadState::GameState_LoadState(const sf::Window& window)
	...
	m_loadedCursesWindow(new SFMLCursesWindow(window, sf::Vector2i(0,0))),
	m_border(nullptr),
...
void GameState_LoadState::OnKeyPressed(sf::Keyboard::Key key, bool alt, bool control, bool shift)
{
	...
	case sf::Keyboard::Return:
		if(m_filename.length() > 0)
		{
			...
			{
				m_messages.push_back(new SFMLStateMessage_PopState(std::unique_ptr<StateInfo_CursesWindowLoadResult>(new StateInfo_CursesWindowLoadResult(true, std::move(m_loadedCursesWindow)))));
			}
		}
		break;
